...
// ===== UI SHIMS (inline, no external UI) =====
function clsx(...xs){ return xs.filter(Boolean).join(' '); }

function Button({ variant='default', className='', children, ...props }){
  const base = 'inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-medium transition shadow-sm ring-1 ring-black/5';
  const styles = variant==='secondary' ? 'bg-white text-gray-900 hover:bg-gray-50' : 'bg-emerald-600 text-white hover:bg-emerald-700';
  return <button className={clsx(base, styles, className)} {...props}>{children}</button>;
}
function Card({ className='', children, ...props }){ return <div className={clsx('bg-white rounded-2xl ring-1 ring-black/5 shadow-md', className)} {...props}>{children}</div>; }
function CardHeader({ className='', children }){ return <div className={clsx('p-5 pb-3', className)}>{children}</div>; }
function CardTitle({ className='', children }){ return <div className={clsx('text-lg font-bold', className)}>{children}</div>; }
function CardContent({ className='', children }){ return <div className={clsx('p-5', className)}>{children}</div>; }
function Input({ className='', ...props }){ return <input className={clsx('w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500', className)} {...props} />; }
function Label({ className='', children }){ return <label className={clsx('text-sm font-medium', className)}>{children}</label>; }
function Badge({ className='', children }){ return <span className={clsx('inline-flex items-center rounded-full px-2 py-0.5 text-xs', className)}>{children}</span>; }

function Accordion({ children }){ return <div>{children}</div>; }
function AccordionItem({ children }){ return <details className="rounded-xl border border-gray-200 bg-white p-3 open:shadow-sm">{children}</details>; }
function AccordionTrigger({ children, className='' }){ return <summary className={clsx('cursor-pointer select-none text-sm font-medium', className)}>{children}</summary>; }
function AccordionContent({ children }){ return <div className="mt-2 text-sm text-gray-600">{children}</div>; }

function Select({ value, onValueChange, children }){ 
  return <select value={value} onChange={(e)=>onValueChange?.(e.target.value)} className="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500">{children}</select>;
}
function SelectContent({ children }){ return <>{children}</>; }
function SelectGroup({ children }){ return <>{children}</>; }
function SelectLabel({ children }){ return <option disabled>— {children} —</option>; }
...
function Slider({ value=[0], min=0, max=100, step=1, onValueChange, className='' }){
  const v = value?.[0] ?? 0;
  return <input type="range" min={min} max={max} step={step} value={v} onChange={(e)=>onValueChange?.([Number(e.target.value)])} className={clsx('w-full', className)} />;
}

// ===== MOTION & ICON SHIMS (no external deps) =====
const motion = { h1: ({children, ...rest}) => <h1 {...rest}>{children}</h1> };
function makeIcon(glyph){
  return function Icon({ size=16, className='' }){
    return <span className={className} style={{ fontSize: size, lineHeight: 1 }} aria-hidden="true">{glyph}</span>;
  };
}
const Upload = makeIcon('⤓');
const MessageCircle = makeIcon('💬');
const ChevronRight = makeIcon('›');
const Ruler = makeIcon('📏');
const Layers = makeIcon('🧱');
const Zap = makeIcon('⚡');
const Calculator = makeIcon('🧮');
const CheckCircle2 = makeIcon('✔️');
...
// ===== Компоненты =====
function QuickCallModal({ open, onClose }){
  ...
  if(!open) return null;
  const submit=()=>{
    const nv=isValidName(name), pv=isValidPhone(phone);
    const human=Date.now()-startRef.current>2000, hpOk=!hp;
    if(!(nv&&pv&&agree&&human&&hpOk)){
      const r=[]; if(!nv) r.push('имя (≥2 буквы)'); if(!pv) r.push('телефон +7 (XXX) XXX-XX-XX'); if(!agree) r.push('согласие'); if(!human||!hpOk) r.push('антибот');
      setError('Проверьте: '+r.join(', ')); return;
    }
    setError('');
    submitLead({ lead_id:genLeadId(), type:'callback', consent:agree, contact:{name,phone}, utm:getUTM() });
    onClose?.();
  };
  return (
    <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm">
      <div className="absolute left-1/2 top-1/2 w-[92%] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-xl ring-1 ring-black/10">
        <h3 className="text-lg font-bold">Заказать звонок за 30 сек</h3>
        <div className="mt-4 space-y-2">
          <Input placeholder="Иванов Иван" value={name} onChange={e=>setName(e.target.value)} />
          <Input placeholder="+79161234567" value={phone} onChange={e=>setPhone(formatPhone(e.target.value))} onFocus={()=>{ if(!phone) setPhone('+'); }} inputMode="tel" />
          <input type="text" tabIndex="-1" autoComplete="off" value={hp} onChange={e=>setHp(e.target.value)} className="hidden" aria-hidden="true" />
          <div className="flex items-start gap-2 text-xs text-gray-600"><input type="checkbox" checked={agree} onChange={e=>setAgree(e.target.checked)} className="mt-1"/><span>Согласен на обработку персональных данных (ФЗ‑152).</span></div>
          {error && <p className="text-xs text-red-600">{error}</p>}
        </div>
        <div className="mt-4 flex gap-2">
          <Button className="bg-emerald-600 text-white hover:bg-emerald-700" onClick={submit}>Отправить</Button>
...
function App(){
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50/50 to-white text-gray-900">
      ...
      <header className="sticky top-0 z-50 border-b border-black/5 bg-white/80 backdrop-blur">
        <Container className="flex h-16 items-center justify-between">
          ...
          <div className="hidden items-center gap-2 md:flex">
            <Button className="bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => document.getElementById('upload')?.scrollIntoView({behavior:'smooth'})}>Загрузить модель</Button>
            <Button variant="secondary" className="bg-white" onClick={()=>setCallOpen(true)}>Заказать звонок за 30 сек</Button>
          </div>
        </Container>
      </header>
        ...
        <Container className="grid gap-8 pb-10 pt-10 md:grid-cols-2 md:pb-16 md:pt-16">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Button variant="secondary" className="bg-white" onClick={()=>setCallOpen(true)}>Заказать звонок</Button>
            </div>
          </div>
      ...
      {/* Materials teaser */}
      <section id="cases" className="bg-white py-12">
        <Container>
          <h2 className="mb-6 text-2xl font-extrabold tracking-tight">Кейсы</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Card className="overflow-hidden rounded-2xl border-0 shadow-md ring-1 ring-black/5"><CardContent className="p-0"><div className="aspect-[16/9] w-full"><AssetImg asset="case-auto" alt="Автообвес"/></div><div className="p-5"><div className="mb-1 font-bold">Автообвес: бампер и антикрыло</div><p className="text-sm text-gray-600">ASA, подготовка и покраска</p></div></CardContent></Card>
            <Card className="overflow-hidden rounded-2xl border-0 shadow-md ring-1 ring-black/5"><CardContent className="p-0"><div className="aspect-[16/9] w-full"><AssetImg asset="case-panels" alt="Параметрические панели"/></div><div className="p-5"><div className="mb-1 font-bold">Параметрические панели 1.8 м</div><p className="text-sm text-gray-600">FGF, секции, подсветка</p></div></CardContent></Card>
            <Card className="overflow-hidden rounded-2xl border-0 shadow-md ring-1 ring-black/5"><CardContent className="p-0"><div className="aspect-[16/9] w-full"><AssetImg asset="case-industrial" alt="Промышленные детали"/></div><div className="p-5"><div className="mb-1 font-bold">Шнек/импеллер, PA+CF</div><p className="text-sm text-gray-600">Высокая прочность</p></div></CardContent></Card>
          </div>
        </Container>
      </section>
